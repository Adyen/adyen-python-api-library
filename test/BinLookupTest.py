...
REQUEST_KWARGS = {"merchantAccount": "YourMerchantAccount", "amount": "1000"}
...
class TestBinLookup(unittest.TestCase):
    ...
    binLookup_version = ady.binlookup.bin_lookup_api.baseUrl.split("/")[-1]
    ...
    def test_get_cost_estimate_success(self):
        ...
        result = self.ady.binlookup.bin_lookup_api.get_cost_estimate(REQUEST_KWARGS)
    ...
    def test_get_cost_estimate_error_mocked(self):
        self.ady.client = self.test.create_client_from_file(
            status=200,
            request=REQUEST_KWARGS,
            filename=("test/mocks/binlookup/getcostestimate-error-invalid-data-422.json"),
        )
